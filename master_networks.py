...
class Master_Gen(nn.Module):
    # AdaIN auto-encoder architecture
    def __init__(self, input_dim, params):
        super(Master_Gen, self).__init__()
        dim = params['dim']
        style_dim = params['style_dim']
        n_downsample = params['n_downsample']
        n_res = params['n_res']
...
class CIN_StyleEncoder(nn.Module):
    def __init__(self, n_downsample, dom_num, nlatent, input_dim, dim, style_dim, norm, activ, pad_type):
        ...
        for i in range(2):
            self.cin_con += [CIN_Conv2dBlock(dim, 2 * dim, 4, 2, 1, norm=norm, activation=activ, pad_type=pad_type, nlatent=nlatent)]
            dim *= 2
...
class CIN_Conv2dBlock(nn.Module):
    def __init__(self, input_dim ,output_dim, kernel_size, stride,
                 padding=0, norm='none', activation='relu', pad_type='zero', nlatent=16):
        super(CIN_Conv2dBlock, self).__init__()
        # weight type mismatch??
        # at line 482,504
        self.use_bias = True
        # initialize padding
        if pad_type == 'reflect':
        ...
        # initialize normalization
        norm_dim = output_dim
        self.norm = CondInstanceNorm(norm_dim,z_dim=nlatent).cuda()
...
class InstanceNorm(nn.Module): # torch.nn.InstanceNorm2d is based on BatchNorm and is different from standard implementation
    def __init__(self, num_features, affine=True, eps=1e-5):
        ...
        self.shift = nn.Parameter(torch.Tensor(num_features))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--config', type=str, default='configs/style_shoes_label_folder.yaml', help='Path to the config file.')
    parser.add_argument('--output_path', type=str, default='.', help="outputs path")
    parser.add_argument("--resume", action="store_true")
    parser.add_argument('--trainer', type=str, default='MUNIT', help="MUNIT|UNIT")
    opts = parser.parse_args()
    config = get_config(opts.config)
    model = Master_Gen(config['input_dim']+config['dom_num'],config['gen']).cuda()
    # print(model)

    x = torch.randn(1,3,256,256).cuda()
    print(x.shape)
    dom_code = torch.zeros(1,2,256,256).cuda()
    dom_code[:,0,:,:]= torch.ones(1,256,256).cuda()
    x = torch.cat((x,dom_code),1)
    print(x.shape)
    images_fake = model(x)
    print(images_fake.shape)
    # g_domainess = nn.Linear(2,16)
    # inp = torch.randn(1,2,1,1)
