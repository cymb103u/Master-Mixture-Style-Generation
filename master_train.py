...
"""
from utils import get_all_data_loaders, prepare_sub_folder, write_html, write_loss, get_config, write_2images, Timer, domain_code_produce_encoder, domain_code_produce_decoder, get_domainess
import argparse
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--config', type=str, default='configs/edges2handbags_folder.yaml', help='Path to the config file.')
    parser.add_argument('--output_path', type=str,
                        default='.', help="outputs path")
    parser.add_argument('--resume', action="store_true")
    parser.add_argument('--trainer', type=str,
                        default='MASTER', help="MASTER|MUNIT|UNIT")
    ...
    while True:
        for it, (images_a, images_b) in enumerate(zip(train_loader_a, train_loader_b)):
            ...
            z_style = get_domainess(
                iterations, max_iter, config['batch_size'], distribution_type=config['distribution_sample']).detach()
            ...
            if (iterations + 1) % config['image_display_iter'] == 0:
                with torch.no_grad():
                    image_outputs = trainer.sample(
                        train_display_images_a, train_display_images_b)
                    images_interp = trainer.interpolation_sample(
                        test_display_images_a, test_display_images_b)
